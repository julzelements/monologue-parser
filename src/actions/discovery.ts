...
export const MINILOGUE_SEARCH_REQUEST_INITIATED =
  "MINILOGUE_SEARCH_REQUEST_INITIATED";
export const MINILOGUE_SEARCH_REQUEST_CANCELED =
  "MINILOGUE_SEARCH_REQUEST_CANCELED";
export const MINILOGUE_SEARCH_REPLY_RECEIVED =
 ...
 * @param {String} port - The MIDI output port ID.
 */
export const initiateMinilogueSearchRequest =
  (port) => (dispatch, getState) => {
    const { discovery: discoveryState, midi } = getState();
    const { pendingSearchRequests: pendingSearchRequestsState } =
      discoveryState;
...
type Midi = {
  ports: [{ type: string; id: any }];
}

/**
 * Create actions to initiate Minilogue device searches in on all attached MIDI outputs.
 */
export const discoverMinilogueDevices = () => (dispatch, getState) => {
  const midi: Midi = getState().midi;
  const { ports } = midi;
  Object.values(ports)
