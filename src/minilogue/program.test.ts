...
test("that the init program title is parsed correctly", () => {
  const decoded = program.decodeProgram(program.INIT_PATCH);
  expect(
...
test("that the init program cross mod depth is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.CROSS_MOD_DEPTH]
  ).toBe(0);
});

test("that the init program pitch EG int is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.VCO2_PITCH_EG_INT]
  ).toBe(512);
});
...
test("that the init program noise level is parsed correctly", () => {
  expect(program.decodeProgram(program.INIT_PATCH)[program.NOISE_LEVEL]).toBe(
    0
  );
});
...
test("that the init program filter cutoff EG int is parsed correctly", () => {
  expect(program.decodeProgram(program.INIT_PATCH)[program.CUTOFF_EG_INT]).toBe(
    512
  );
});

test("that the init program filter type is parsed correctly", () => {
  expect(program.decodeProgram(program.INIT_PATCH)[program.CUTOFF_TYPE]).toBe(
    1
  ); // 4-pole
});
...
test("that the init program filter key tracking is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.CUTOFF_KEYBOARD_TRACK]
  ).toBe(0);
});

test("that the init program filter type is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.CUTOFF_VELOCITY]
  ).toBe(0);
});
...
test("that the init program amp EG attack is parsed correctly", () => {
  expect(program.decodeProgram(program.INIT_PATCH)[program.AMP_EG_ATTACK]).toBe(
    0
  );
});

test("that the init program amp EG decay is parsed correctly", () => {
  expect(program.decodeProgram(program.INIT_PATCH)[program.AMP_EG_DECAY]).toBe(
    512
  );
});
...
test("that the init program amp EG sustain type is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.AMP_EG_SUSTAIN]
  ).toBe(1023);
});

test("that the init program amp EG release is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.AMP_EG_RELEASE]
  ).toBe(0);
});
...
test("that the init program delay hi pass cutoff type is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.DELAY_HI_PASS_CUTOFF]
  ).toBe(256);
});

test("that the init program delay time is parsed correctly", () => {
  expect(program.decodeProgram(program.INIT_PATCH)[program.DELAY_TIME]).toBe(
    1023
  );
});
...
test("that the init program delay feedback is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.DELAY_FEEDBACK]
  ).toBe(1023);
});

test("that the init program delay output routing is parsed correctly", () => {
  expect(
    program.decodeProgram(program.INIT_PATCH)[program.DELAY_OUTPUT_ROUTING]
  ).toBe(0); // Bypass
});
